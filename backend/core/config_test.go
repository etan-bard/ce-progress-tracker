...
func TestConfigGetScriptBatchSize(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			batchSize:   500,
			expected:    500,
			description: "Batch size of 500 should be returned as-is",
		},
		{
			name:        "batch size too small",
			batchSize:   0,
			expected:    500,
			description: "Batch size of 0 should default to 500",
		},
		{
			name:        "batch size too large",
			batchSize:   501,
			expected:    500,
			description: "Batch size of 501 should default to 500",
		},
		{
			name:        "negative batch size",
			batchSize:   -1,
			expected:    500,
			description: "Negative batch size should default to 500",
		},
		{
			name:        "very large batch size",
			batchSize:   1000,
			expected:    500,
			description: "Batch size of 1000 should default to 500",
		},
	}
